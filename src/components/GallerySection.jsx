...
export default function GallerySection() {
  ...
  return (<section id="gallery" className="py-24 bg-coffee-50 dark:bg-coffee-950 text-coffee-950 dark:text-white relative">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      {/* <div className="flex justify-center gap-3 mb-12 flex-wrap">
        {
          ["all", "brews", "ambiance", "bakery"].map(cat => (<button key={cat} onClick={() => setFilter(cat)} className={`px-5 py-2 rounded-full font-bold text-xs capitalize transition ${
            filter === cat
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {
          filtered.map(item => (<div key={item.id} onClick={() => setActiveImage(item)} className="relative h-72 rounded-3xl overflow-hidden glass-card border border-coffee-200 dark:border-coffee-800 group cursor-pointer">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-coffee-950 via-coffee-950/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6 text-white">
              <span className="text-[10px] uppercase tracking-widest text-amber-300 font-extrabold">
                {item.category}
              </span>
              <h3 className="text-lg font-serif font-bold mt-1">
                {item.title}
              </h3>
              <p className="text-xs text-coffee-200 mt-1 line-clamp-2">
                {item.caption}
              </p>
            </div>
