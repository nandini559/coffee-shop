...
import { Search, Star, ShoppingBag, Info, X, ShieldAlert, Sparkles, AlertCircle } from 'lucide-react';
...
export default function MenuSection() {
  ...
  return (
    <section id="menu" className="py-24 bg-[#FDFBF7] dark:bg-[#160F0B] text-[#2C1A14] dark:text-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <h2 className="text-4xl sm:text-5xl font-serif font-bold text-[#2C1A14] dark:text-white mt-3">
            Our Handcrafted <span className="text-[#C67C4E] dark:text-[#F0C085]">Menu</span>
          </h2>
        ...
        <div className="max-w-2xl mx-auto mb-10 space-y-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search espresso, cold brew, matcha, sourdough, croissant..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-12 pr-4 py-3.5 rounded-2xl bg-white dark:bg-[#160F0B] border border-[#E5DCD3] dark:border-coffee-800 focus:border-[#C67C4E] text-[#2C1A14] dark:text-white text-sm font-medium placeholder-[#855E4C] focus:outline-none transition shadow-sm"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-coffee-400 hover:text-coffee-900"
              >
                <X className="w-4 h-4" />
              </button>
            )}
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            <span className="text-xs font-extrabold text-[#855E4C] dark:text-coffee-400 mr-1 uppercase tracking-wider">Dietary:</span>
            {tagsList.map(tag => (
              <button
                key={tag}
                ...
                className={`px-3 py-1 rounded-full text-xs font-bold border transition ${
                  selectedTag === tag
                    ...
                    : 'bg-white dark:bg-coffee-950/60 border-[#E5DCD3] dark:border-coffee-800 text-[#3D291F] dark:text-coffee-300 hover:border-[#C67C4E]'
                }`}
              >
                {tag}
              </button>
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={`relative px-5 py-3 rounded-2xl font-extrabold text-xs sm:text-sm tracking-wider uppercase transition-colors ${
                  isActive
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-16 glass-card rounded-3xl">
            <p className="text-[#5C4337] dark:text-coffee-300 text-base font-medium">No items matched your search criteria.</p>
            <button
              onClick={() => { setActiveCategory('all'); setSearchQuery(''); setSelectedTag(''); }}
                ...
                return (
                  <motion.div
                    ...
                    className={`glass-card rounded-3xl overflow-hidden border flex flex-col justify-between group transition-all duration-300 ${
                      isOutOfStock ? 'opacity-85 border-red-500/30' : 'border-[#E5DCD3] dark:border-coffee-800 hover:border-[#C67C4E] dark:hover:border-amber-400/30'
                    }`}
                  >
                    <div>
                      <div className="relative h-52 overflow-hidden">
                        ...
                        <motion.button
                          ...
                          className="absolute top-4 right-4 p-2 rounded-full bg-white/90 dark:bg-[#160F0B]/80 hover:bg-[#2C1A14] dark:hover:bg-[#C67C4E] hover:text-white dark:hover:text-[#160F0B] text-[#2C1A14] dark:text-white transition backdrop-blur-md shadow-sm"
                          title="Quick Details"
                        >
                      ...
                      <div className="p-6">
                        ...
                        <div className="mt-4 flex flex-wrap gap-1.5">
                          {item.tags.map(t => (
                            <span key={t} className="text-[10px] font-extrabold bg-[#FDFBF7] dark:bg-coffee-950 px-2 py-0.5 rounded-md text-[#2C1A14] dark:text-[#F0C085] border border-[#E5DCD3] dark:border-coffee-800">
                              {t}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="p-6 pt-0 flex items-center justify-between border-t border-[#E5DCD3] dark:border-coffee-800">
                      <div>
                        <span className="text-xs text-[#855E4C] dark:text-coffee-400 block font-bold uppercase tracking-wider">Price</span>
                      ...
                      <motion.button
                        ...
                        className={`px-4 py-2.5 rounded-xl font-extrabold text-xs uppercase tracking-wider transition shadow-sm flex items-center gap-1.5 ${
                          isOutOfStock
                      ...
                      >
                        <ShoppingBag className="w-4 h-4" /> {isOutOfStock ? 'Sold Out' : 'Add to Order'}
                      </motion.button>
                    </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
            ...
            >
              ...
              <button
                ...
                className="absolute top-4 right-4 p-2 rounded-full bg-[#FDFBF7] dark:bg-coffee-900 text-[#2C1A14] dark:text-coffee-300 hover:text-black"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="grid grid-cols-2 gap-4 bg-[#FDFBF7] dark:bg-[#160F0B] p-4 rounded-2xl border border-[#E5DCD3] dark:border-coffee-800 text-xs">
                <div>
                  <span className="font-extrabold text-[#2C1A14] dark:text-[#F0C085] block mb-1">Key Ingredients:</span>
                ...
                <div>
                  <span className="font-extrabold text-[#2C1A14] dark:text-[#F0C085] block mb-1 flex items-center gap-1">
                    <ShieldAlert className="w-3.5 h-3.5 text-[#C67C4E] dark:text-[#F0C085]" /> Allergens:
                  </span>
                  <p className="text-[#5C4337] dark:text-coffee-300 font-medium">{activeItemModal.allergens.join(', ')}</p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-amber-500/10 p-4 rounded-2xl border border-[#E5DCD3] dark:border-amber-500/20 text-xs space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-extrabold text-[#2C1A14] dark:text-amber-200">Recommended Pairing:</span>
                  <span className="font-bold text-[#5C4337] dark:text-white">{activeItemModal.pairingRecommendation}</span>
                </div>
              </div>

              <div className="flex items-center justify-between pt-4 border-t border-[#E5DCD3] dark:border-coffee-800">
                <span className="text-2xl font-black text-[#2C1A14] dark:text-[#F0C085]">${activeItemModal.price.toFixed(2)}</span>
                <motion.button
                  whileHover={!outOfStockMenuItemIds.includes(activeItemModal.id) ? { scale: 1.05 } : {}}
                  whileTap={!outOfStockMenuItemIds.includes(activeItemModal.id) ? { scale: 0.95 } : {}}
                  disabled={outOfStockMenuItemIds.includes(activeItemModal.id)}
                  onClick={() => {
                    addToCart(activeItemModal);
                    setActiveItemModal(null);
                  }}
                  className={`px-6 py-3 rounded-2xl font-extrabold text-xs uppercase tracking-wider shadow-sm flex items-center gap-2 ${
                    outOfStockMenuItemIds.includes(activeItemModal.id)
                      ? 'bg-gray-300 dark:bg-gray-800 text-gray-500 cursor-not-allowed'
                      : 'bg-[#2C1A14] dark:bg-[#C67C4E] text-white dark:text-[#160F0B]'
                  }`}
                >
                  <ShoppingBag className="w-4 h-4" /> {outOfStockMenuItemIds.includes(activeItemModal.id) ? 'Sold Out' : 'Add to Order'}
                </motion.button>
              </div>
