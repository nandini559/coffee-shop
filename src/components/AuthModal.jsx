...
export default function AuthModal() {
  ...
  return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
    <motion.div initial={{
        opacity: 0,
        scale: 0.9,
        y: 20
      ...
      {/* Close Button */}
      <button onClick={() => setIsAuthModalOpen(false)} className="absolute top-4 right-4 p-2 rounded-full bg-[#FAF7F2] dark:bg-coffee-900 text-[#2C1A14] dark:text-coffee-300 hover:text-black">
        <X className="w-5 h-5"/>
      </button>
        ...
        user
          ? (<div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              <img src={user.avatar} alt={user.name} className="w-16 h-16 rounded-2xl object-cover border-2 border-[#C67C4E] shadow-sm"/>
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-xl font-serif font-bold text-[#2C1A14] dark:text-white">
                    {user.name}
                  </h3>
                  {
                    user.role === "admin" && (<span className="px-2 py-0.5 rounded-md bg-[#2C1A14] text-white text-[10px] font-black uppercase">
                      Admin
                    </span>)
                  }
                </div>
            ...
            <div className="grid grid-cols-2 gap-3 bg-[#FDFBF7] dark:bg-[#160F0B] p-4 rounded-2xl border border-[#E5DCD3] dark:border-coffee-800 text-xs">
              <div>
                <span className="text-[#855E4C] dark:text-coffee-400 font-bold block">
                  Reward Points
                </span>
              ...
              <div>
                ...
                <span className="text-sm font-bold text-[#2C1A14] dark:text-white mt-1 block">
                  {user.memberSince}
                </span>
              </div>
            </div>
              ...
              <div className="space-y-1.5">
                {
                  user.favoriteDrinks && user.favoriteDrinks.length > 0
                    ? (user.favoriteDrinks.map(drink => (<div key={drink} className="bg-white dark:bg-coffee-900/60 px-3 py-2 rounded-xl border border-[#E5DCD3] dark:border-coffee-800 text-xs font-bold text-[#2C1A14] dark:text-white flex items-center justify-between">
                      <span className="flex items-center gap-2">
                        <Coffee className="w-3.5 h-3.5 text-[#C67C4E]"/>{" "}
                        {drink}
                      </span>
                      <CheckCircle2 className="w-4 h-4 text-emerald-500"/>
                    </div>)))
                    : (<p className="text-xs text-[#855E4C] dark:text-coffee-400 italic">
                      No favorite drinks saved yet.
                    </p>)
                }
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-2 p-1 bg-[#FDFBF7] dark:bg-coffee-900 rounded-2xl border border-[#E5DCD3] dark:border-coffee-800">
              <button onClick={() => setAuthTab("login")} className={`py-2 rounded-xl text-xs font-extrabold transition ${
                authTab === "login"
                  ? "bg-[#2C1A14] dark:bg-[#C67C4E] text-white dark:text-[#160F0B] shadow-sm"
                  : "text-[#855E4C] dark:text-coffee-300"}`}>
                Sign In
              </button>
              <button onClick={() => setAuthTab("signup")} className={`py-2 rounded-xl text-xs font-extrabold transition ${
                authTab === "signup"
                  ? "bg-[#2C1A14] dark:bg-[#C67C4E] text-white dark:text-[#160F0B] shadow-sm"
                  : "text-[#855E4C] dark:text-coffee-300"}`}>
                Sign Up
            ...
            <div className="p-3 rounded-2xl bg-white dark:bg-coffee-900/60 border border-[#E5DCD3] dark:border-coffee-800 text-xs space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => {
                  ...
                  <span className="font-bold text-[#2C1A14] dark:text-white block text-[11px]">
                    {DEMO_USER.name}
                  </span>
                  <span className="text-[9px] text-[#855E4C] dark:text-coffee-400">
                    Customer Account
                  </span>
                </button>
              ...
              authTab === "login"
                ? (<form onSubmit={handleLoginSubmit} className="space-y-4">
                  <div>
                    ...
                    <input type="email" required="required" placeholder="e.g. sarah@oakandbean.coffee" value={formData.email} onChange={e => setFormData(prev => ({
                        ...prev,
                  ...
                  <div>
                    ...
                    <input type="password" required="required" placeholder="••••••••" value={formData.password} onChange={e => setFormData(prev => ({
                        ...prev,
                  ...
                  <button type="submit" className="w-full py-3.5 rounded-xl bg-[#2C1A14] dark:bg-[#C67C4E] text-white dark:text-[#160F0B] font-extrabold text-xs uppercase tracking-wider shadow-sm flex items-center justify-center gap-2">
                ...
                : (<form onSubmit={handleSignupSubmit} className="space-y-4">
                  <div>
                    ...
                    <input type="text" required="required" placeholder="e.g. Michael Scott" value={formData.name} onChange={e => setFormData(prev => ({
                        ...prev,
                        name: e.target.value
                      }))
} className="w-full px-4 py-2.5 rounded-xl bg-white dark:bg-[#160F0B] border border-[#E5DCD3] dark:border-coffee-800 text-xs font-medium focus:border-[#C67C4E] focus:outline-none"/>
                  ...
                  <div>
                    ...
                    <input type="email" required="required" placeholder="e.g. michael@example.com" value={formData.email} onChange={e => setFormData(prev => ({
                        ...prev,
                        email: e.target.value
                      }))
} className="w-full px-4 py-2.5 rounded-xl bg-white dark:bg-[#160F0B] border border-[#E5DCD3] dark:border-coffee-800 text-xs font-medium focus:border-[#C67C4E] focus:outline-none"/>
